 ...
 *
 * Canonical source of truth for session validation across all packages.
 * Prevents malicious JSON injection, DoS attacks, and invalid data
 * from being processed by the dev server, API endpoints, and CLI.
 *
 * Previously lived in @gremlin/cli/src/session-validation.ts and was
 * duplicated as manual validation in @gremlin/server-shared/src/types.ts.
 */
...
export const GremlinEventSchema = z.object({
 ...
 */
export function validateSessionZod(data: unknown): GremlinSession {
